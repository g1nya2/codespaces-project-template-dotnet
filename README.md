...
이 프로젝트는 쉽게 커스텀할 수 있습니다. 사이트의 각 섹션은 별도의 컴포넌트이며, 사용자의 정보는 한 곳에만 저장해야 합니다. 이렇게 함으로써 업데이트를 쉽게 할 수 있고, 어떤 식으로 Blazor 컴포넌트에 값을 전달하는지 확인할 수 있습니다.
...
_선택 사항_ 에는 이메일 주소와 소셜 계정 입력이 있습니다. 이들은 `Footer` 컴포넌트에 사용됩니다. 목록에 포함되지 않거나 빈 문자열("")로 설정된 경우 해당 아이콘과 링크가 표시되지 않습니다.

### 2️⃣ 이미지 업데이트하기

이 포트폴리오 사이트에는 세 가지 이미지가 포함되어 있습니다 : 상단 배경, "About me" 배경, 포트폴리오 섹션(책상 그림). 이 기본 세가지 이미지들 대신 사용 허락이 없어도 자유롭게 사용할 수 있는 이미지나, 자신이 소유하고 있는 이미지 중 **가로 형태**의 이미지를 넣어도 됩니다.
