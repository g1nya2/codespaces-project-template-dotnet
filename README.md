...
이 포트폴리오 사이트에는 세 가지 이미지가 포함되어 있습니다 : 상단 배경, "About me" 배경, 포트폴리오 섹션(책상 그림). 이 기본 세가지 이미지들 대신 사용 허락이 없어도 자유롭게 사용할 수 있는 이미지나, 자신이 소유하고 있는 이미지 중 **가로 형태**의 이미지를 넣으면 됩니다.
